    ...
    return (
        <nav className={`sticky top-0 z-50 transition-all duration-500 ${scrolled ? 'py-2' : 'py-4'}`}>
            <div className={`mx-auto max-w-7xl px-4 md:px-8 transition-all duration-500 rounded-full border border-white/40 shadow-sm hover:shadow-lg ${scrolled ? 'bg-white/70 backdrop-blur-xl w-[95%]' : 'bg-white/30 backdrop-blur-lg w-[98%]'} flex items-center justify-between`}>
                ...
                {/* Nav Links (Mega Menu) */}
                <div className={`hidden md:flex items-center gap-8 font-medium text-sm text-gray-700 ${showSearch ? 'opacity-0 pointer-events-none absolute' : 'opacity-100 relative'}`}>
                    {NAV_MENU.map((item) => (
                ...
                {showSearch && (
                    <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 w-1/2">
                        <form onSubmit={handleSearch} className="w-full relative">
                            ...
                            />
                            <button type="button" onClick={() => setShowSearch(false)} className="absolute right-3 top-1/2 -translate-y-1/2 p-1 hover:bg-gray-200 rounded-full">
                                <X size={16} />
                            </button>
                        </form>
                    </div>
                ...
                <div className="hidden md:flex items-center gap-2 pr-2">
                    ...
                    >
                        <Search className={`w-5 h-5 group-hover:text-indigo-600 ${showSearch ? 'text-white group-hover:text-white' : 'text-gray-700'}`} strokeWidth={2} />
                    </button>
