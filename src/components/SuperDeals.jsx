...
export default function SuperDeals({ hotDrops = [], flashSale = [], flashSaleEndsAt = null }) {
    return (
        <section className="max-w-7xl mx-auto px-4 md:px-8 py-6 grid grid-cols-1 md:grid-cols-2 gap-6 pb-8 border-b border-gray-100 font-sans">
            ...
            <div className="bg-white p-2">
                ...
                <div className="grid grid-cols-3 gap-3">
                    {flashSale.length > 0 ? (
                        flashSale.slice(0, 3).map((product) => (
                            <Link
                            ...
                            >
                                ...
                                <div className="absolute bottom-0 left-0 w-full p-2 bg-gradient-to-t from-black/70 to-transparent pt-6">
                                    <div className="font-bold text-sm text-white">
                                        Rs. {product.pricing?.salePrice}
                                    </div>
                                </div>
                            </Link>
                        ))
                    ) : (
                        <div className="col-span-3 text-center text-gray-400 text-xs py-10">
                            No flash sale items yet.
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="bg-white p-2">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-black italic flex items-center text-black tracking-tight">
                        Hot Drop
                        <span className="ml-2 text-[9px] font-bold not-italic bg-[#FFE5CC] text-[#FF5F00] px-1.5 py-0.5 rounded-[2px]">
                            New Styles
                        </span>
                    </h3>
                    <Link href="/products?sort=new" className="text-xs text-gray-500 hover:text-black flex items-center">
                        View All <ChevronRight size={14} />
                    </Link>
                </div>
